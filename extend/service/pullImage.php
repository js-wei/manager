...
class pullImage{
    ...
    public function download_current_page_images($capture_url)
    {
        ...
        if(strpos('img',$content)!==true){
            return $this->save_image($content,$capture_url);
            exit;
        }
     ...
     */
    private function save_image($img_data,$capture_url){
        $pathinfo = pathinfo($capture_url);    //获取图片路径信息        
        $pic_name=$pathinfo['basename'];   //获取图片的名字
       
        if(strlen($pic_name)<2){
            $n = pathinfo($pathinfo['dirname']);
            $pic_name=$n['basename'];
        }
        $suffix  = explode(',',$_SERVER['HTTP_ACCEPT']);
        ...
        $fullPath = $this->save_path ."/". $pic_name.".{$suffix}";
        ...
        $img_size = file_put_contents($fullPath, $img_data);
        if ($img_size) {
           return array(
            'filePath'=>$fullPath,
            'fileName'=> $pic_name.".{$suffix}"
...
//$download_img=new pullImage('E:/images/',0);                              //实例化下载图片对象
