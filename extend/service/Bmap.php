...
class Bmap{
	private $url=array(
        ...
        'iplookup'=>'http://apistore.baidu.com/microservice/iplookup?ip=',
     ...
     */
    public function ip(){
        $url=$this->url['ip'].'ip='.get_client_ip().'&coor=bd09ll'.$this->param; 
        $data=$this->_get($url);
        return $data;
    }
    /**
     * [get_location 根据IP获取地理位置]
     * @param  string $id [IP]
     * @return [type]     [description]
     */
    public function get_location($ip=''){
        $url=$this->url['iplookup'].$ip;
        $data=$this->_get($url);
        return $data;
    }

    public function place_eventsearch(){
     ...
     * @param  string  $type     [区域类型：boundslat：lng(左下角坐标),lat,lng(右上角坐标)，radius(r)lat<纬度>,lng<经度>+周边检索半径，单位为米]
   ...
   /* private function _filter($filter='宾馆|价格,200-300'){
    ...
    public function weather($location=array('徐州','苏州')){
        ...
        foreach ($data['results'] as $k => $v) {
            ...
            $_result=$this->add_weather($v['currentCity']);
            $v['pinyin']=$_result['retData']['pinyin'];     
            //$v['citycode']=$_result['retData']['citycode'];
            $v['postCode']=$_result['retData']['postCode'];     
            $v['longitude']=$_result['retData']['longitude'];   
            $v['latitude']=$_result['retData']['latitude'];
            $v['pm25_aqi']=$_result['retData']['aqi'];              //空气质量指数
            $v['pm25_level']=$_result['retData']['level'];          //空气等级
            $v['pm25_core']=$_result['retData']['core'];            //首要污染物
            $v['pm25_time']=$_result['retData']['pm_time'];         //数据采集时间
            $v['sunrise']=$_result['retData']['sunrise'];           //日出时间
            $v['sunset']=$_result['retData']['sunset'];             //日落时间
            ksort($v);
            $results[]=$v;
        }
     ...
     */
    private function add_weather($cityname=''){
        $url='http://apistore.baidu.com/microservice/weather?cityname='.urlencode($cityname);
        $url1='http://apistore.baidu.com/microservice/aqi?city='.urlencode($cityname);
        $data=$this->_get($url);
        $pm=$this->_get($url1);
       
       
        $data['retData']['aqi']=$pm['retData']['aqi'];
        $data['retData']['level']=$pm['retData']['level'];
        $data['retData']['core']=$pm['retData']['core'];
        $data['retData']['pm_time']=$pm['retData']['time'];
        ksort($data);
        return $data;
    }
